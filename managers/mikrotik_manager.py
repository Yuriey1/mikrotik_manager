...
class MikroTikManager:
            ...
            for addr in addresses:
                try:
                    ...
                    if ip_obj in network:
                        print(f"✅ IP {ip} принадлежит сети {network} на интерфейсе '{interface}'")
    ...
    def add_static_arp(self, ip: str, mac: str, comment: str = "", interface: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if interface is None:
                ...
                if not belongs:
                    raise ValueError(f"IP {ip} не принадлежит сетям микротика! ARP запись не может быть добавлена.")
